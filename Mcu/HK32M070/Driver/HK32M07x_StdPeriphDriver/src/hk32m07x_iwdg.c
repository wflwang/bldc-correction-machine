...
/** @defgroup IWDG IWDG
  ...
  * @{
    ...
    [..] Min-max timeout value @40KHz (LSI): ~0.1ms / ~28.3s
         The IWDG timeout may vary due to LSI frequency dispersion. hk32m07x
         ...
         For more information, please refer to the hk32m07x Reference manual.
...
/** @defgroup IWDG_Group1 Prescaler_And_Counter_Configuration Prescaler_And_Counter_Configuration
...
/** @defgroup IWDG_Group2 IWDG_Activation IWDG_Activation
...
/** @defgroup IWDG_Group3 Flag_Management Flag_Management
...
/** @defgroup IWDG_Group4 Interrupts_And_Flags_Fanagement Interrupts_And_Flags_Fanagement
...
/**
  * @brief  Checks whether the specified IWDG interrupt has occurred or not.
  * @param  IWDG_IT: specifies the IWDG interrupt source to check.
  *         This parameter can be one of the following values:
  *         IWDG_IRQCFG_IWDG_IRQF:
...
FlagStatus IWDG_GetITStatus(uint32_t IWDG_IT)
{
    FlagStatus bitstatus = RESET;
    /* Check the parameters */
    assert_param(IS_IWDG_IRQEN(IWDG_IT));

    if ((IWDG->IRQCFG & IWDG_IRQCFG_IWDG_IRQF) != (uint16_t)RESET)
    {
        bitstatus = SET;
    }
    else
...
{
    IWDG->IRQCFG &= (uint32_t)~IWDG_IRQCFG_IWDG_IRQF;
}

/**
  * @brief  Sets and Enable the IWDG IT value.
  * @param  Time
  * @retval None
  */
...
{
    /* Check the parameters */
    assert_param(IS_IWDG_WINDOW_VALUE(Time));
    ...
    IWDG->KR = 0x5555;
    IWDG->IRQCFG = (Time & 0xFFF) | IWDG_IRQCFG_IWDG_IRQEN;
}
