...
void HTU_IRQHandler(void){
    ...
    else if (((flag & (uint16_t)HTU_IT_SW) != (uint16_t)RESET))  
    {
        //数据变化中断
        //HTU->HIFR = flag;
        //HTU->HIFR  = (uint16_t)HTU_IT_SW;
...
static void M_HALL_TIMx_CC_IRQHandler( void * pHandleVoid )
{
	...
	int16_t ang_hall_int;
	int16_t tempL_int_diff;
	//int16_t tempR_int_diff;
    ...
    uint8_t hall_val1 = GPIO_IsInputPinSet( HW_HALL_ENC_GPIO1, HW_HALL_ENC_PIN1 ) << 2
                        | GPIO_IsInputPinSet( HW_HALL_ENC_GPIO2, HW_HALL_ENC_PIN2 ) << 1
                        | GPIO_IsInputPinSet( HW_HALL_ENC_GPIO3, HW_HALL_ENC_PIN3 );
    uint8_t count=0;
    ...
    switch(pHandle->hallState){
        case hall_run: //正常模式
            ...
            if(m_ang60_intTime>=MaxAng60IntTime){   //如果上次超时了 认为速度过慢/堵转了 重新开始hall 角度
                ...
                if(DirCMPint16(ang_diff,pHandle->last_ang_diff)==0){   //误差变化方向是否一致 
                    //误差方向一致
                    if(abs_ang_diff>hEdegree(80)){    //>80度 认为只变化80度
                        //本次角度变化过大 限制更新角度 本次误差如果>80度 可能是上次60度变化丢失了 duty限制要降下来
                        pHandle->m_ang_hall_int_prev += SignCMPint16(ang_diff,hEdegree(80)); 
                    }else if(abs_ang_diff<hEdegree(40)){  //<40度 直接认为变化60度吗？
                        pHandle->m_ang_hall_int_prev += SignCMPint16(ang_diff,hEdegree(60)); 
                    }else{
                    ...
                    pHandle->last_ang_diff = ang_diff;    //更新误差 主要是更新误差方向 下次 方向就可以对了
        ...
        default:    //第二次触发到开始记录 记录当前角度和相对上次变化的角度 推测下一个可能的角度
            ...
            //计算电角度的变化
            ang_diff = (int)nowEAngle - (int)lastHallEAngle; //上次角度 - 本次角度 = 角度的变化值
			if (ang_diff > 32767)
				ang_diff -= 65536;
			else if (ang_diff < -32768)
				ang_diff += 65536;
            ...
            //记住当前角度
            foc_hall_ang_Temptable[hall_val] = foc_hall_ang_Temptable[hall_val]+(int)nowEAngle;    //赋值当前电角度
            lastHallEAngle = nowEAngle;
            pHandle->hallState++;
			//3(+1),4,5,6,7,8(+6)  9,10,11,12,13,14
			//return;
            if(pHandle->hallState>=8){
                //4轮了 每个点有4轮角度数据 取平均
                for(uint8_t i=0;i<8;i++){
                    pHandle->foc_hall_table[i] = (int16_t)(foc_hall_ang_Temptable[i]);    //逐个更新hall table
                }
                //结束hall学习
