/**
  ...
  *          of the Ramp Extended Manager component
  *****************************************************************************************************
**/
/* Includes ------------------------------------------------------------------*/
...
#ifdef HWDIV
  RCC->AHBENR |= RCC_AHBPeriph_DIVSQ;
#endif
...
#ifdef HWDIV
  ret_val = DVSQ_QWKDivsion(pHandle->Ext, ( int32_t )( pHandle->ScalingFactor ),NULL );
#else
...
bool REMNG_ExecRamp( RampExtMngr_Handle_t * pHandle, int32_t TargetFinal, uint32_t Durationms )
{
  ...
  /* Get current state */
#ifdef HWDIV
  current_ref = DVSQ_QWKDivsion(pHandle->Ext, ( int32_t )( pHandle->ScalingFactor ),NULL );
#else
