...
void UTU2_IRQHandler(void)
{
    ...
    {
		//上升沿中断
        //UTU_CLK->UIFR = UTU_IT_UCRA;
        PWM_L = UTU_GetCapture(UTU_CLK, UTU_ICSelection_IO_SELF);
        //响应了一个完整的周期
        if(cnt<lostPWMCnt)
	    cnt++;
    }
	if(temp & UTU_IT_UCFA){
		//下降沿中断 有数据更新
        //UTU_CLK->UIFR = UTU_IT_UCFA;
        updateReady = 1;    //有更新
        PWM_H = UTU_GetCapture(UTU_CLK, UTU_ICSelection_IO_SELF);
	}
	if(temp & UTU_IT_OVF){
		//最大时间溢出了 可能是最大 也可能是最小
        //超时也是完整周期
        //UTU_CLK->UIFR = UTU_IT_OVF;
        updateReady = 1;    //有更新
        PWM_H = 0;
        lastPPM = 0;        //掉码
		cnt = 0;
    ...
    UTU_ClearFlag(UTU_CLK, (uint16_t)temp);
}
/**
 * @brief 获取本次PPM的值
 * 
