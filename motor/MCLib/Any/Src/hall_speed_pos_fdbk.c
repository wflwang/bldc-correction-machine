...
void HALL_Init( HALL_Handle_t * pHandle )
{
    ...
    pHandle->OvfFreq = ( uint16_t )( pHandle->TIMClockFreq / 16777216u );
    ...
    pHandle->HALLMaxRatio = 1 ;

    /* Align MaxPeriod to a multiple of Overflow.*/
    pHandle->MaxPeriod = ( pHandle->HALLMaxRatio ) * 16777216u;
...
void HALL_Clear( HALL_Handle_t * pHandle )
{
    ...
    HTU_ITConfig(HTU_IT_SW | HTU_IT_ERR | HTU_IT_OVF, DISABLE);
    ...
    HTU_SetCounter( HALL_COUNTER_RESET );
